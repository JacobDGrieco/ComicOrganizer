...
from . import list_order, missing_entries, organizer, reindex_output
...
def run_flatten(config_path: Path, *, apply: bool, paths: list[str] | None) -> int:
	...
	config = load_config(config_path)
	flatten_paths = paths or [str(source_folder.path) for source_folder in config.source_folders]
	command = [
		"powershell",
		"-NoProfile",
		"-ExecutionPolicy",
...
def run_downloader(config_path: Path, *, headless: bool) -> int:
	...
	output_folder = optional_config_path(raw_config, config_folder, "download_output_folder")
...
def optional_config_path(raw_config: dict[str, object], config_folder: Path, key: str) -> Path | None:
...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Run ComicOrganizer commands for one project profile.")
	parser.add_argument(
		"command",
		choices=("sort", "missing", "list", "reindex", "flatten", "download", "characters", "db-validate", "db-export"),
	)
	parser.add_argument("--project", help="Project folder under projects/, such as spider-man or x-men.")
	parser.add_argument("--config", help="Explicit config path. Overrides --project.")
	parser.add_argument("--dry-run", action="store_true", help="Dry-run sort operations.")
	parser.add_argument("--apply", action="store_true", help="Apply reindex or flatten operations.")
	parser.add_argument("--headless", action="store_true", help="Run downloader browser in hidden mode.")
	parser.add_argument("--path", action="append", help="Flatten this path instead of configured source folders.")
